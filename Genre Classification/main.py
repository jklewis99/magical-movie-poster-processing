...
def class_activation_map(img_path, model_type):
    ...
    # Get the final convolution layer. The index is determine by the following code: dictionary = {v.name: i for i, v in enumerate(model.layers)}
    if model_type == '1':
        layer_index = 1038
    elif model_type == '2':
        layer_index = 779
    ...
    if model_type == '':
        ...
        if mode == 'train':
            ...
            if path == '':
                ...
                class_activation_map(path, model_type)
        else:
            print('Invalid Mode')
